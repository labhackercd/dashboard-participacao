...
import { ENQUETES_GOOGLE_ANALYTICS } from "../../config_constants";
...
const last7days = {
  ...
  query: {
    ...
    "start-date": "7daysAgo",
    "end-date": "yesterday",
    filters: "ga:pagePath=@/ex/enquetes/",
  },
  chart: {
    type: "LINE",
    options: {
...
export default class EnquetesAnalyticsPage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      token: "",
  ...
  async componentDidMount() {
    try {
      const response = await fetch(ENQUETES_GOOGLE_ANALYTICS); // Alterar pela URL válida
      ...
      case "month":
        return (
          <GoogleAnalyticsCharts
            views={views}
            title={"Enquetes - últimos 30 dias"}
  ...
  render() {
    ...
    if (loading) {
      ...
      return (
        <ResponsiveDrawer title="Analytics Enquetes">
